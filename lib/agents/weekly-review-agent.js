...
export async function runWeeklyReviewAgent(apiKey, {
  ...
  const dayRows = weekDays.map(d => {
    ...
    return `${d.report_date} | Score:${d.total_score ?? '?'} | Stores:${d.stores_visited ?? '?'} | CheckIn:${d.resumption_time || '?'} | Brands:${brandsHit}/${activeBrands.length}(${bPct}%) | ₦${Number(d.value_of_orders || 0).toLocaleString()} | Debt:${d.debt_score ?? '?'}/30`;
  }).join('\n');
