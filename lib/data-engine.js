...
/**
 * Product name aliases: maps known PepUp product names to their canonical brand partner name.
 * e.g. B-boon's product is sold under the name "BisQuate" in PepUp reports.
 */
const BRAND_ALIASES = {
  'bisquate': 'B-boon',
  'felon innovation ltd': 'Felon',
};
...
/**
 * Resolve a brand name from the product report to its canonical partner name.
 * Checks aliases first, then returns original if no alias found.
 */
function resolveAlias(brandName) {
  const lower = brandName.toLowerCase().trim();
  const withoutBp = lower.replace(/^bp\s+/, '').trim();
  return BRAND_ALIASES[lower] || BRAND_ALIASES[withoutBp] || brandName;
}

export function resolveCanonicalBrand(brandName, activeBrands = []) {
  const resolved = resolveAlias(String(brandName || '').trim());
  const resolvedNorm = normaliseBrandName(resolved);
  if (!resolvedNorm) return null;

  for (const brand of activeBrands) {
    if (normaliseBrandName(brand) === resolvedNorm) return brand;
  }

  for (const brand of activeBrands) {
    const canonicalNorm = normaliseBrandName(brand);
    if (
      (canonicalNorm.length >= 4 && resolvedNorm.includes(canonicalNorm)) ||
      (resolvedNorm.length >= 4 && canonicalNorm.includes(resolvedNorm))
    ) {
      return brand;
    }
  }
 ...
 * Known product aliases (e.g. BisQuate → B-boon) are resolved before matching.
 */
export function buildBrandCoverage(productData, activeBrands) {
  ...
  for (const [name, entry] of productData) {
    ...
    for (const brandRow of entry.brandRows) {
      if (!brandRow.value) continue;

      const canonicalBrand = resolveCanonicalBrand(brandRow.brand, activeBrands);
      if (!canonicalBrand) continue;
      row[canonicalBrand] = true;
    }
...
export function processReports(checkinBuffer, productBuffer, feedbackDetailBuffer, seRoster, activeBrands) {
  ...
  for (const seName of seRoster) {
    ...
    const brandsOrdered   = activeBrands.filter(brand => coverage[brand]).length;
...
/**
 * Name lookup with progressive fallbacks.
 *
 * 1. Exact map key match (fastest path, handles identical strings).
 * 2. Case-insensitive + whitespace-normalised exact match.
 * 3. Word-set match: all meaningful words (>2 chars) from the shorter name
 *    must appear in the longer name — handles reordered or middle-initial
 *    differences (e.g. "John A. Doe" ↔ "John Doe", "Doe John" ↔ "John Doe").
 *    Requires at least 2 words to match to prevent false positives on
 *    SEs who share only a first name.
 *
 * NOTE: The old fallback matched on first name only, which caused wrong data
 * to be assigned when multiple SEs share a first name (e.g. "Emmanuel Obi"
 * getting "Emmanuel Adeyemi"'s checkin/product data).
 */
function findEntry(map, name) {
  ...
  // Pass 2: case + punctuation insensitive exact match
  for (const [key, val] of map) {
    if (normalise(key) === normName) return val;
  }

  // Pass 3: word-set match — requires ALL words of the shorter name to appear
  // in the longer name, and at least 2 words must participate in the match.
  const nameWords = new Set(normName.split(/\s+/).filter(w => w.length > 2));
  if (nameWords.size < 2) return null; // single-word name — too ambiguous

  for (const [key, val] of map) {
    const keyWords = new Set(normalise(key).split(/\s+/).filter(w => w.length > 2));
    if (keyWords.size < 2) continue;

    const smaller = nameWords.size <= keyWords.size ? nameWords : keyWords;
    const larger  = nameWords.size <= keyWords.size ? keyWords  : nameWords;

    if ([...smaller].every(w => larger.has(w))) return val;
  }
