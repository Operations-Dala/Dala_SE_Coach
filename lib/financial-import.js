...
function findHeaderIndexAny(row, ...labels) {
  return row.findIndex(cell => labels.includes(normaliseHeader(cell)));
}
...
function parseExpensePepUpRows(rows, rosterRows, fallbackDate) {
  const DATE_ALIASES   = ['allowance date', 'expense date', 'activity date'];
  const AMOUNT_ALIASES = ['total amount', 'expense amount', 'admin approved amount'];

  const headerIndex = rows.findIndex(row =>
    Array.isArray(row) &&
    row.some(cell => normaliseHeader(cell) === 'salesman name') &&
    row.some(cell => DATE_ALIASES.includes(normaliseHeader(cell))) &&
    row.some(cell => AMOUNT_ALIASES.includes(normaliseHeader(cell)))
  );
  ...
  const dateIndex   = findHeaderIndexAny(header, ...DATE_ALIASES);
  const amountIndex = findHeaderIndexAny(header, ...AMOUNT_ALIASES);
