...
export async function generateCoaching(apiKey, {
  ...
  const metricsSummary  = buildMetricsSummary(se, teamAvg, totalSEs);
...
function buildMetricsSummary(se, teamAvg, totalSEs) {
  return `
Resumption Time:  ${se.resumption_time || 'Unknown'}
Stores Visited:   ${se.stores_visited} (team avg: ${teamAvg.stores_visited?.toFixed(1)})
Brands Ordered:   ${se.brands_ordered} (team avg: ${teamAvg.brands_ordered?.toFixed(1)})
