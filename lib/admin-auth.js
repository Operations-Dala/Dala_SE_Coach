...
export function requireSameOrigin(request) {
  ...
  // No Origin header → allow. Browsers always include Origin for cross-origin
  // requests, so a real CSRF attack cannot hide behind a missing Origin.
  if (!requestOrigin) {
    return null;
  }
...
function getRequestOrigin(request) {
  // Explicit override — set NEXT_PUBLIC_APP_URL on Railway (or any proxy) when
  // the host header may reflect the internal container address rather than the
  // public hostname (e.g. NEXT_PUBLIC_APP_URL=https://myapp.railway.app).
